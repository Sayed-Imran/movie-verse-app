#!/usr/bin/env python3
"""
Script to fetch 20 Hollywood movies from each genre and store them in JSON files.
...
DATA_DIR = Path(os.path.dirname(os.path.abspath(__file__))) / "hollywood-data"

# Create data directory if it doesn't exist
os.makedirs(DATA_DIR, exist_ok=True)

# Path to the genres.json file
GENRES_FILE = Path(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))) / "frontend" / "genres.json"

async def fetch_from_tmdb(endpoint: str, params: dict = None):
    """Helper function to fetch data from TMDB API with Hollywood parameters."""
    if params is None:
        params = {}
    
    # Add parameters for English/US movies
    params.update({
...
async def fetch_genre_movies(genre_id: int, genre_name: str, page: int = 1, results_per_genre: int = 20):
    ...
    data = await fetch_from_tmdb("/discover/movie", {
        ...
        "vote_count.gte": 100  # To filter for more popular Hollywood movies
    })
...
async def fetch_all_genres_movies():
    ...
    try:
        ...
        print(f"Found {len(genres)} genres. Starting to fetch Hollywood movies...")
        ...
        print("\nAll Hollywood genre data has been successfully fetched and stored!")
...
if __name__ == "__main__":
    ...
    print("Starting to fetch Hollywood movies by genre...")
